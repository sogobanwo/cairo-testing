...
fn test_transfer_ownership_works () {
    ...
    assert(new_owner == new_owner_address, 'Owner transfer fails');
}

// test if transfer ownership by a fake admin works
#[test]
#[should_panic(expected : ('Caller is not the owner', ))]
fn test_fake_admin_transfer_ownership_fails() {
    ...
    assert( new_owner_address== new_owner, 'Owner transfer fails');
}
